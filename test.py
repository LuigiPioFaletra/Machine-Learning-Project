...
if __name__ == '__main__':
    ...
    # Load the pre-trained model weights for the CNN, FNN and SVM models
    cnn_model.load_state_dict(torch.load(f'{config.training.checkpoint_dir}/{config.best.cnn}', weights_only=True))
    print('CNN Model loaded.')
    fnn_model.load_state_dict(torch.load(f'{config.training.checkpoint_dir}/{config.best.ff}', weights_only=True))
    ...
    models = [('CNN', cnn_model), ('FNN', fnn_model)]                   # List of models to evaluate
    ...
    # After evaluate CNN and FNN models, use the FNN model to extract features on test set for SVM
    fnn_model.eval()
    with torch.no_grad():
        test_features = fnn_model(test_emb_tensor.to(device)).cpu().numpy()
