...
from model_classes.fnn_model import FNNAudioClassifier
...
if __name__ == '__main__':
    ...
    # Instantiate the CNN and FNN models
    cnn_model = CNNAudioClassifier(config.model.input_size,
                                   config.model.num_classes,
                                   config.model.dropout).to(device)
    fnn_model = FNNAudioClassifier(config.model.input_size,
    ...
    fnn_optimizer = torch.optim.Adam(fnn_model.parameters(), lr=config.training.learning_rate, weight_decay=config.training.weight_decay)
    ...
    fnn_scheduler = torch.optim.lr_scheduler.LambdaLR(fnn_optimizer, lr_lambda=scheduler_lambda)

    # List of models to train
    models = [('CNN', cnn_model, cnn_optimizer, cnn_scheduler, config.best.cnn),
              ('FNN', fnn_model, fnn_optimizer, fnn_scheduler, config.best.ff)]
    ...
    # After training CNN and FNN models, use the FNN model to extract features on training and validation sets for SVM
    fnn_model.eval()
    with torch.no_grad():
        train_features = fnn_model(torch.tensor(train_emb, dtype=torch.float32).to(device)).cpu().numpy()
        val_features = fnn_model(torch.tensor(val_emb, dtype=torch.float32).to(device)).cpu().numpy()
